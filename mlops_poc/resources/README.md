...
The databricks CLI bundles top level is defined by file `mlops_poc/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`mlops_poc/resources/model-workflow-resource.yml`)
 - batch inference workflow (`mlops_poc/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`mlops_poc/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`mlops_poc/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`mlops_poc/resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-mlops-poc-model     | /dev-mlops-poc-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest develop content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-mlops-poc-model | /staging-mlops-poc-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest main content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-mlops-poc-model    | /prod-mlops-poc-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the develop branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-mlops-poc-model    | /test-mlops-poc-experiment    |
...
You can open a PR (pull request) to modify ML code or the resource config against develop branch.
...
Upon merging a PR to the develop branch, the develop branch content will be deployed to the staging workspace with `staging` environment resource configurations.
...
2. Complete the `TODO` in `mlops_poc/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Open a pull request to merge the pushed branch into the `develop` branch.
...
Once CI passes, merge the PR into the `develop` branch. This will deploy an initial set of Databricks resources to the staging workspace.
Resources will be deployed to the prod workspace on pushing code to the `main` branch.
...
Following this, update the batch_inference_job base parameters in `mlops_poc/databricks-resource/batch-inference-workflow-resource.yml` to pass
...
To get started, open `mlops_poc/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-mlops-poc-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-mlops-poc-batch-inference-job`
that runs the notebook under `mlops_poc/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-mlops-poc-batch-inference-job`.
...
`test-mlops-poc-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-mlops-poc-batch-inference-job`.
...
After setting up CI/CD, PRs against the develop branch will trigger CI workflows to run unit tests, integration test and resource validation.
...
After merging a PR to the develop branch, continuous deployment automation will deploy the `staging` resources to the staging workspace.

When you about to cut a release, you can create and merge a PR to merge changes from develop to main. Continuous deployment automation will deploy `prod` resources to the prod workspace.
