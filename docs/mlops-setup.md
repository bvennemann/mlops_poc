...
[ML Config](../mlops_poc/resources/README.md)  guides to make changes to ML code or deployed jobs.
...
Commit the current `README.md` file and other docs to the `develop` branch of the repo, to enable forking the repo:
```
git add README.md docs .gitignore mlops_poc/resources/README.md
...
* You must be a Databricks workspace admin in the staging and prod workspaces. Verify that you're an admin by viewing the
  [staging workspace admin console](https://adb-8195009858693383.3.azuredatabricks.net#setting/accounts) and
  [prod workspace admin console](https://adb-4437901305756982.2.azuredatabricks.net#setting/accounts). If
...
After the pull request merges, pull the changes back into your local `develop`
branch:
...
Create and push a release branch called `main` off of the `develop` branch of the repository:
```
...
Your production jobs (model training, batch inference) will pull ML code against this branch, while your staging jobs will pull ML code against the `develop` branch. Note that the `develop` branch will be the source of truth for ML resource configs and CI/CD workflows.

For future ML code changes, iterate against the `develop` branch and regularly deploy your ML code from staging to production by merging code changes from the `develop` branch into the `main` branch.
## Deploy ML resources and enable production jobs
Follow the instructions in [mlops-poc/resources/README.md](../mlops_poc/resources/README.md) to deploy ML resources
and production jobs.
...
[ML pull request guide](ml-pull-request.md) and [ML resource config guide](../mlops_poc/resources/README.md)  to propose, test, and deploy
